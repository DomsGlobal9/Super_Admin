...
import { Search, ChevronDown, AlertTriangle, FileText, CreditCard, RefreshCw, Shield } from 'lucide-react';
...
const Notifications = () => {
  ...
  const notifications = [
    ...
    },
    {
      ...
      description: 'Q3 reports pending for the following departments in Admin',
      time: '6 minutes',
      bgColor: 'bg-green-50',
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h2 className="text-3xl md:text-2xl font-semibold text-gray-900 mb-1">Notifications Center</h2>
          <p className="text-sm text-gray-600">Manage alerts and system notifications</p>
        </div>

        {/* Stats Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 h-28 gap-3 md:gap-5 mb-6">
  {stats.map((stat, index) => (
    ...
    >
      <div className="text-md text-gray-600 mb-1">{stat.label}</div>
      <div className={`text-lg md:text-1xl font-bold ${stat.color}`}>{stat.value}</div>
    </div>
  ))}
</div>
          ...
          <div className="p-3 md:p-4">
            <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
              ...
              <div className="relative">
                <button
                  className="flex items-center gap-2 px-3 py-1.5 text-sm border border-gray-300 rounded-lg bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  onClick={() => setFilterBy(filterBy === 'All' ? 'Urgent' : 'All')}
                >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="divide-y divide-gray-200">
            {notifications.map((notification) => (
              <div
                key={notification.id}
                className={`p-3 md:p-4 ${notification.bgColor} border-l-4 ${notification.borderColor} hover:bg-opacity-80 transition-colors`}
              >
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 p-1.5 rounded-md bg-white shadow-sm">
        ...
        {/* <div className="mt-4 text-center">
          <button className="px-4 py-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 focus:outline-none">
            Load more
          </button>
        </div> */}
      </div>
